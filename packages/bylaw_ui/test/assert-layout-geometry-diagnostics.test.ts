...
test("alignment diagnostics identify the rule, subject, reference, and aligned edge", () => {
  ...
  const f = {
    ...
    reference: "card",
    relationship: "align",
    expected: { alignment: "left", tolerancePx: 0 },
    actual: { subjectCoordinate: 12, referenceCoordinate: 10, differencePx: 2 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 4, passed: 3, failed: 1, skipped: 0 },
    findings: [f],
  }).message;
  for (const s of [
    "rule 3",
...
test("alignment diagnostics report both measured coordinates in CSS pixels", () => {
  ...
  const f = {
    ...
    expected: { alignment: "left", tolerancePx: 0 },
    actual: {
      subjectCoordinate: 12.5,
      referenceCoordinate: 10.25,
      differencePx: 2.25,
    },
  };
  const m = new LayoutAssertionError({
  ...
  expect(m).toContain("subject coordinate: 12.5px");
  expect(m).toContain("reference coordinate: 10.25px");
});
...
test("tolerance diagnostics report zero pixels when tolerance is omitted", () => {
  ...
  const f = {
    ...
    actual: { subjectWidthPx: 8, referenceWidthPx: 10, widthDifferencePx: 2 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
...
test("ordering violation diagnostics report the signed gap and boundary crossing in CSS pixels", () => {
  ...
  expect(m).toContain("signed gap: -2px");
  expect(m).toContain("boundary crossing: 2px");
});
...
test("ordering violation diagnostics report the allowed tolerance and excess boundary crossing", () => {
  ...
  expect(m).toContain("allowed tolerance: 1px");
  expect(m).toContain("exceeds tolerance by: 1px");
});
...
test("bounded gap diagnostics report the measured gap and both allowed endpoints", () => {
  ...
  expect(m).toContain("measured gap: 2px");
  expect(m).toContain("allowed gap: 4px–8px");
});
...
test("bounded gap diagnostics report distance below the minimum endpoint", () => {
  ...
  expect(
    new LayoutAssertionError({
      passed: false,
      rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
      findings: [f],
    }).message,
  ).toContain("below minimum by: 2px");
});
...
test("bounded gap diagnostics report distance above the maximum endpoint", () => {
  ...
  ).toContain("above maximum by: 2px");
});
...
test("minimum-only gap diagnostics report how far the measurement is below the minimum", () => {
  ...
  const f = {
    ...
    reference: "b",
    relationship: "leftOf",
    expected: { gap: { minPx: 4 } },
    actual: { signedGapPx: 1.5 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  expect(m).toContain("allowed gap: at least 4px");
  expect(m).toContain("below minimum by: 2.5px");
});
...
test("maximum-only gap diagnostics report how far the measurement exceeds the maximum", () => {
  ...
  const f = {
    ...
    reference: "b",
    relationship: "leftOf",
    expected: { gap: { maxPx: 8 } },
    actual: { signedGapPx: 10.5 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  expect(m).toContain("allowed gap: at most 8px");
  expect(m).toContain("above maximum by: 2.5px");
});
...
test("missing overlap diagnostics report horizontal and vertical overlap depths including zeroes", () => {
  ...
  expect(m).toContain("horizontal overlap: 0px");
  expect(m).toContain("vertical overlap: 10px");
});
...
test("missing overlap diagnostics state that positive overlap is required on both axes", () => {
  ...
  ).toContain("required overlap: positive on both axes");
});
...
test("constrained overlap diagnostics report measured depths and each configured range", () => {
  ...
  const f = {
    ...
    expected: {
      ...
      vertical: { minPx: 6, maxPx: 10 },
    },
    actual: { horizontalPx: 2, verticalPx: 12 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  for (const s of [
    ...
    "vertical overlap: 12px",
    "allowed vertical overlap: 6px–10px",
  ])
    expect(m).toContain(s);
});
...
test("constrained overlap diagnostics report the distance from every violated range boundary", () => {
  ...
  expect(m).toContain("horizontal below minimum by: 2px");
  expect(m).toContain("vertical above maximum by: 2px");
});
...
test("minimum-only overlap diagnostics report how far the depth is below the minimum", () => {
  ...
  const f = {
    ...
    reference: "b",
    relationship: "overlap",
    expected: { horizontal: { minPx: 4 } },
    actual: { horizontalPx: 2, verticalPx: 10 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  expect(m).toContain("allowed horizontal overlap: at least 4px");
  expect(m).toContain("horizontal below minimum by: 2px");
});
...
test("maximum-only overlap diagnostics report how far the depth exceeds the maximum", () => {
  ...
  const f = {
    ...
    reference: "b",
    relationship: "overlap",
    expected: { horizontal: { maxPx: 6 } },
    actual: { horizontalPx: 8, verticalPx: 10 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  expect(m).toContain("allowed horizontal overlap: at most 6px");
  expect(m).toContain("horizontal above maximum by: 2px");
});
...
test("overlap diagnostics do not report a range violation for a valid axis", () => {
  ...
  const f = {
    ...
    actual: { horizontalPx: 2, verticalPx: 8 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  expect(m).toContain("horizontal below minimum by: 2px");
  expect(m).not.toContain("vertical below minimum");
  expect(m).not.toContain("vertical above maximum");
});
...
test("notOverlap diagnostics report positive overlap depths on both axes", () => {
  ...
  const f = {
    ...
    subject: "a",
    reference: "b",
    relationship: "notOverlap",
    expected: { overlap: false },
    actual: { horizontalPx: 5, verticalPx: 10 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  expect(m).toContain("horizontal overlap: 5px");
  expect(m).toContain("vertical overlap: 10px");
});
...
test("notOverlap diagnostics state that positive overlap is prohibited", () => {
  ...
  ).toContain("allowed overlap: none");
});
...
test("containment diagnostics report overflow on all four sides including zeroes", () => {
  ...
  const f = {
    ...
    reference: "b",
    relationship: "inside",
    expected: { tolerancePx: 0, positiveIntersection: true },
    actual: {
      leftPx: 2,
      rightPx: 0,
      topPx: 3,
...
test("containment diagnostics do not report excess for edges within tolerance", () => {
  ...
  expect(m).toContain("left exceeds tolerance by: 1px");
  expect(m).not.toContain("right exceeds tolerance");
});
...
test("sameHeight diagnostics report subject height, reference height, and absolute difference", () => {
  ...
  const f = {
    ...
    subject: "a",
    reference: "b",
    relationship: "sameHeight",
    expected: { tolerancePx: 0 },
    actual: {
      subjectHeightPx: 718,
      referenceHeightPx: 720,
      heightDifferencePx: 2,
    },
  };
  const m = new LayoutAssertionError({
    passed: false,
...
test("sameSize diagnostics do not report excess for a dimension within tolerance", () => {
  ...
  expect(m).toContain("width exceeds tolerance by: 1px");
  expect(m).not.toContain("height exceeds tolerance");
});
...
test("geometry diagnostics preserve fractional CSS-pixel measurements without unnecessary trailing zeroes", () => {
  ...
  const f = {
    ...
    reference: "b",
    relationship: "sameWidth",
    expected: { tolerancePx: 0.25 },
    actual: {
      subjectWidthPx: 98.5,
      referenceWidthPx: 100,
      widthDifferencePx: 1.5,
    },
  };
  const m = new LayoutAssertionError({
    passed: false,
  ...
  for (const s of ["98.5px", "100px", "1.5px", "0.25px", "1.25px"])
    expect(m).toContain(s);
...
test("geometry diagnostics render negative zero as zero pixels", () => {
  ...
  const f = {
    ...
    relationship: "leftOf",
    expected: { tolerancePx: 0 },
    actual: { signedGapPx: -0, boundaryCrossingPx: 0 },
  };
  const m = new LayoutAssertionError({
    passed: false,
    rules: { total: 1, passed: 0, failed: 1, skipped: 0 },
  ...
  expect(m).toContain("signed gap: 0px");
  expect(m).not.toContain("-0px");
});
...
test("geometry diagnostics preserve exact calculated violation amounts", () => {
  ...
  expect(message).toContain("exceeds tolerance by: 9007199254740991px");
});
