  ...
  const report = await checkLayout({
    adapter: createAdapter({
      measure: async () => ({
        ...
        targets: [resolved("panel", { x: 0.25, y: 0.5, width: 10.75, height: 20.5 })],
      }),
    }),
...
test("a synchronously thrown adapter failure preserves the original error identity", async () => {
  ...
  const promise = checkLayout({ adapter, rules: [width("panel", { minPx: 1 })] });
  expect(await promise.catch((error: unknown) => error)).toBe(original);
});
...
test("an asynchronously rejected adapter failure preserves the original error identity", async () => {
  ...
  const adapter = createAdapter({ measure: async () => { throw original; } });
  const promise = checkLayout({ adapter, rules: [width("panel", { minPx: 1 })] });
  expect(await promise.catch((error: unknown) => error)).toBe(original);
});
...
test("a measurement validation failure is distinct from an adapter platform failure", async () => {
  ...
  const adapter = createAdapter({ measure: async () => { throw platform; } });
...
for (const [name, mutate] of [
  ...
  ["rejects a snapshot without viewport data before rule evaluation", () => ({ targets: validSnapshot().targets })],
  ["rejects a snapshot without target results before rule evaluation", () => ({ viewport: validSnapshot().viewport })],
  ["rejects a nonarray target result collection before rule evaluation", () => ({ ...validSnapshot(), targets: {} })],
  ["rejects nonpositive viewport dimensions before rule evaluation", () => ({ ...validSnapshot(), viewport: { width: 0, height: 100 } })],
  ["rejects nonfinite viewport dimensions before rule evaluation", () => ({ ...validSnapshot(), viewport: { width: Number.NaN, height: 100 } })],
  ["rejects a nonobject target result before rule evaluation", () => ({ ...validSnapshot(), targets: [null, validSnapshot().targets[1]] })],
  ["rejects a target result with a nonstring identifier before rule evaluation", () => ({ ...validSnapshot(), targets: [{ ...validSnapshot().targets[0], target: 4 }, validSnapshot().targets[1]] })],
  ["rejects a target result with a negative match count before rule evaluation", () => ({ ...validSnapshot(), targets: [{ target: "subject", matchCount: -1 }, validSnapshot().targets[1]] })],
  ["rejects a target result with a fractional match count before rule evaluation", () => ({ ...validSnapshot(), targets: [{ target: "subject", matchCount: 1.5 }, validSnapshot().targets[1]] })],
  ["rejects a target result with an unsafe match count before rule evaluation", () => ({ ...validSnapshot(), targets: [{ target: "subject", matchCount: Number.MAX_SAFE_INTEGER + 1 }, validSnapshot().targets[1]] })],
  ["rejects a rectangle with a nonfinite coordinate before rule evaluation", () => ({ ...validSnapshot(), targets: [resolved("subject", { x: Number.NaN, y: 0, width: 10, height: 10 }), validSnapshot().targets[1]] })],
  ["rejects a nonobject rectangle before rule evaluation", () => ({ ...validSnapshot(), targets: [{ ...resolved("subject"), rect: null }, validSnapshot().targets[1]] })],
  ["rejects a rectangle with negative dimensions before rule evaluation", () => ({ ...validSnapshot(), targets: [resolved("subject", { x: 0, y: 0, width: -1, height: 10 }), validSnapshot().targets[1]] })],
  ["rejects a rectangle with a nonfinite derived edge before rule evaluation", () => ({ ...validSnapshot(), targets: [resolved("subject", { x: Number.MAX_VALUE, y: 0, width: Number.MAX_VALUE, height: 10 }), validSnapshot().targets[1]] })],
  ["rejects a resolved singular target without element state before rule evaluation", () => ({ ...validSnapshot(), targets: [{ target: "subject", matchCount: 1 }, validSnapshot().targets[1]] })],
  ["rejects an unresolved singular target with element state before rule evaluation", () => ({ ...validSnapshot(), targets: [{ ...resolved("subject"), matchCount: 0 }, validSnapshot().targets[1]] })],
  ["rejects a resolved singular target with a nonboolean hidden state before rule evaluation", () => ({ ...validSnapshot(), targets: [{ ...resolved("subject"), hidden: "no" }, validSnapshot().targets[1]] })],
  ["rejects a collection containing malformed element state before rule evaluation", () => ({ ...validSnapshot(), targets: [{ target: "subject", matches: [{ hidden: false, rect: null }] }, validSnapshot().targets[1]] })],
  ["rejects adapter results that omit a requested target before rule evaluation", () => ({ ...validSnapshot(), targets: [validSnapshot().targets[0]] })],
  ["rejects adapter results that duplicate a requested target before rule evaluation", () => ({ ...validSnapshot(), targets: [validSnapshot().targets[0], validSnapshot().targets[0], validSnapshot().targets[1]] })],
  ["rejects adapter results for an unrequested target before rule evaluation", () => ({ ...validSnapshot(), targets: [validSnapshot().targets[0], resolved("other")] })],
] as const) {
  test(name, async () => {
...
test("malformed adapter results do not partially evaluate otherwise valid rules", async () => {
  const error = await checkLayout({
    ...
    rules: [
      width("valid", { minPx: 100 }),
      width("invalid", { minPx: 1 }),
    ],
